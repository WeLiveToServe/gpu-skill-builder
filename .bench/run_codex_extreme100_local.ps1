...
$log = Join-Path (Join-Path $PWD 'logs') ("codex_extreme100_" + (Get-Date -Format 'yyyyMMdd_HHmmss') + '.log')
...
python .\run_named_suite.py --harness codex --suites extreme100 --timeout-s 1200 --ledger .\suite_runs_codex_extreme100_gpu.json 2>&1 | Tee-Object -FilePath $log -Append
